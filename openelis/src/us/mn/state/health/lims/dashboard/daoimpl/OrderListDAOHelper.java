...
public class OrderListDAOHelper {
    ...
    public String createSqlForToday(String condition, String orderBy, String pendingAnalysisStatus,
    ...
    public String createSqlForPendingBeforeToday(String condition, String orderBy, String pendingAnalysisStatus,
    ...
    private String createSqlStringForTodayOrders(String condition, String OrderBy, String pendingAnalysisStatus,
        ...
        return "SELECT \n" +
                ...
                "CASE WHEN document_track.report_generation_time is null THEN false ELSE true END as is_printed, \n" +
                "CASE WHEN COUNT(analysis.id) = SUM(CASE WHEN  analysis.status_id IN (" +completedStatus+ ") THEN 1 ELSE 0 END) THEN \n" +
                ...
                "LEFT OUTER JOIN document_track as document_track ON sample.id = document_track.row_id AND document_track.name = 'patientHaitiClinical' and document_track.parent_id is null \n" +
                "WHERE " + condition + "\n" +
                "GROUP BY sample.accession_number, sample.uuid,sample.id, sample.collection_date, sample.lastupdated,sample.received_date, person.first_name, person.middle_name, person.last_name, sample_source.name, patient_identity.identity_data, document_track.report_generation_time \n" +
                "ORDER BY " + OrderBy + " DESC\n" +
                "LIMIT 1000;";
    }
    ...
    private String createSqlStringForTodayOrdersANDGroupBySampleEnabled(String condition, String OrderBy, String pendingAnalysisStatus,
    	 ...
    	 return "SELECT \n" +
                 ...
                 "LEFT OUTER JOIN document_track as document_track ON sample.id = document_track.row_id AND document_track.name = 'patientHaitiClinical' and document_track.parent_id is null \n" +
                 "WHERE " + condition + "\n" +
                 "GROUP BY sample.accession_number, sample.uuid,sample.id, sample.collection_date, sample.lastupdated,sample.received_date, person.first_name, person.middle_name, person.last_name, sample_source.name, patient_identity.identity_data, document_track.report_generation_time, type_of_sample.description \n" +
                 "ORDER BY " + OrderBy + " DESC\n" +
                 "LIMIT 1000;";
    }
    

    public Order getOrder(ResultSet accessionResultSet, String comments, String sectionNames, boolean completed, Timestamp completedDate) throws SQLException {
        if (isGroupBySample) {
            return getOrderWithPriority(accessionResultSet, comments, sectionNames, getPriority(accessionResultSet), completed,  completedDate);
        }
        return getOrderWithoutPriority(accessionResultSet, comments, sectionNames, completed, completedDate);
    }
    ...
    private Order getOrderWithPriority(ResultSet accessionResultSet, String comments, String sectionNames,
                                       String priority, boolean completed, Timestamp completedDate) throws SQLException {
    ...
    private Order getOrderWithoutPriority(ResultSet accessionResultSet, String comments,
                                          String sectionNames, boolean completed, Timestamp completedDate) throws SQLException {
